...
const ProfileMyPhotos = () => {
  ...
  const [modalAction, setModalAction] = useState<
    "delete" | "private" | "public" | null
  >(null);
  const [whatIsPrivare, setWhatIsPrivate] = useState<boolean>(false);
  ...
  const handleDelete = async (photoId: number) => {
    try {
      ...
      if (response.data.success) {
        ...
        dispatch(
          setProfilePhotos(photos!.filter((photo) => photo.id !== photoId))
        );
      } else {
  ...
  return (
    <div className="p-4 space-y-4">
      <button
        className="flex gap-2 bg-orange-200 p-1 rounded-full text-black cursor-pointer"
        onClick={() => setWhatIsPrivate(true)}
      >
        <span>Приватні фото</span> <Info />
      </button>
      ...
      {/* Моадальне вікно що таке приватні фото */}
      {whatIsPrivare && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white p-4 rounded-lg shadow-lg w-1/3">
            <h3 className="text-lg font-semibold mb-4 text-black">
              Що таке приватні фото
            </h3>
            <p className="mb-4 text-black">
              Приватні фото - це фото які недоступні користувачам. <br /> Лише
              після того як ви схвалите запит на показ таких фото, - їх буде
              показано користувачу
            </p>
            <div className="flex justify-end">
              <button
                onClick={() => setWhatIsPrivate(false)}
                className="bg-green-300 text-black p-2 rounded-lg"
              >
                Зрозуміло
              </button>
            </div>
